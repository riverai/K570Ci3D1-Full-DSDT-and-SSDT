 ...
 *
 * Disassembly of SSDT4.aml, Thu Oct 29 16:28:48 2015
 *
...
DefinitionBlock ("SSDT4.aml", "SSDT", 1, "HASEE ", "PARADISE", 0x00003000)
{
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Device (B0D3)
        {
            ...
            Method (_INI, 0, NotSerialized)  // _INI: Initialize
            {
                If (LAnd (LNotEqual (ABAR, Ones), LNotEqual (And (ABAR, 0xFFFFC000), 
                    Zero)))
                {
                    Store (ABAR, BARA)
                }
            ...
            Method (ABWA, 1, Serialized)
            {
                If (Arg0)
                {
                    If (LAnd (LNotEqual (And (ABAR, 0xFFFFC004), 0xFFFFC004), LEqual (And (ABAR, 
                        0xFFFFC000), Zero)))
                    {
                        Store (ABAR, TBAR)
                        Store (ACMD, TCMD)
        ...
        Device (IGPU)
        {
            ...
            Device (DD1F)
            {
                ...
                Method (BRUP, 0, NotSerialized)
                {
                    ...
                    Else
                    {
                        If (LLess (Local0, Subtract (PLD2, 0x03)))
                        {
                            Store (PLD2, Local1)
                        }
                        Else
                        {
                            If (LLess (Local0, Subtract (PLD3, 0x03)))
                            {
                                Store (PLD3, Local1)
                            }
                            Else
                            {
                                If (LLess (Local0, Subtract (PLD4, 0x03)))
                                {
                                    Store (PLD4, Local1)
                                }
                                Else
                                {
                                    If (LLess (Local0, Subtract (PLD5, 0x03)))
                                    {
                                        Store (PLD5, Local1)
                                    }
                                    Else
                                    {
                                        If (LLess (Local0, Subtract (PLD6, 0x03)))
                                        {
                                            Store (PLD6, Local1)
                                        }
                                        Else
                                        {
                                            If (LLess (Local0, Subtract (PLD7, 0x03)))
                                            {
                                                Store (PLD7, Local1)
                                            }
                                            Else
                ...
                Method (BRDN, 0, NotSerialized)
                {
                    ...
                    Else
                    {
                        If (LGreater (Local0, Add (PLD6, 0x03)))
                        {
                            Store (PLD6, Local1)
                        }
                        Else
                        {
                            If (LGreater (Local0, Add (PLD5, 0x03)))
                            {
                                Store (PLD5, Local1)
                            }
                            Else
                            {
                                If (LGreater (Local0, Add (PLD4, 0x03)))
                                {
                                    Store (PLD4, Local1)
                                }
                                Else
                                {
                                    If (LGreater (Local0, Add (PLD3, 0x03)))
                                    {
                                        Store (PLD3, Local1)
                                    }
                                    Else
                                    {
                                        If (LGreater (Local0, Add (PLD2, 0x03)))
                                        {
                                            Store (PLD2, Local1)
                                        }
                                        Else
                                        {
                                            If (LGreater (Local0, Add (PLD1, 0x03)))
                                            {
                                                Store (PLD1, Local1)
                                            }
                                            Else
                ...
                Method (LGBR, 1, NotSerialized)
                {
                    ...
                    Else
                    {
                        If (LLessEqual (Arg0, PLD1))
                        {
                            Store (One, Local1)
                        }
                        Else
                        {
                            If (LLessEqual (Arg0, PLD2))
                            {
                                Store (0x02, Local1)
                            }
                            Else
                            {
                                If (LLessEqual (Arg0, PLD3))
                                {
                                    Store (0x03, Local1)
                                }
                                Else
                                {
                                    If (LLessEqual (Arg0, PLD4))
                                    {
                                        Store (0x04, Local1)
                                    }
                                    Else
                                    {
                                        If (LLessEqual (Arg0, PLD5))
                                        {
                                            Store (0x05, Local1)
                                        }
                                        Else
                                        {
                                            If (LLessEqual (Arg0, PLD6))
                                            {
                                                Store (0x06, Local1)
                                            }
                                            Else
                                            {
                                                If (LLessEqual (Arg0, PLD7))
                                                {
                                                    Store (0x07, Local1)
                                                }
                                                Else
                ...
                Method (BRDT, 0, NotSerialized)
                {
                    ...
                    Else
                    {
                        If (LLessEqual (Local0, PLD1))
                        {
                            Store (0x02, Local1)
                        }
                        Else
                        {
                            If (LLessEqual (Local0, PLD2))
                            {
                                Store (0x03, Local1)
                            }
                            Else
                            {
                                If (LLessEqual (Local0, PLD3))
                                {
                                    Store (0x04, Local1)
                                }
                                Else
                                {
                                    If (LLessEqual (Local0, PLD4))
                                    {
                                        Store (0x05, Local1)
                                    }
                                    Else
                                    {
                                        If (LLessEqual (Local0, PLD5))
                                        {
                                            Store (0x06, Local1)
                                        }
                                        Else
                                        {
                                            If (LLessEqual (Local0, PLD6))
                                            {
                                                Store (0x07, Local1)
                                            }
                                            Else
                                            {
                                                If (LLessEqual (Local0, PLD7))
                                                {
                                                    Store (0x08, Local1)
                                                }
                                                Else
